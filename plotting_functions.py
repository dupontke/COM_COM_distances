...
def hist1d(data, x_axis, system, analysis, num_b = 100, norm = True, average = False, t0 = 0, **kwargs):
	""" Creates a 1D histogram:

	Usage: hist1d(data, x_axis, system, analysis, num_b, norm)
	...
	weights: an array of weights, of the same shape as data. If density is True, the weights are normalized, so that the integral over the range is 1. 
	density: [False|True]; If False, the frequency of data; if True, plotting the probability density, normalized such that integral over range is 1.
	average: [False|True]; Default is False; if set to True, the function will calc the average, standard dev, and standard dev of mean of the y-data
	t0: index to begin averaging from; Default is 0
	...
	# INITIATING THE PLOT...
	events, edges, patches = plt.hist(data, bins=num_b, normed=norm, weights=None, density=None, histtype = 'bar')
...
def scat_hist(xdata, ydata, color, x_axis, y_axis, system, analysis, num_b = 100, average = False, t0 = 0, **kwargs):
	...
	"""
	# DEFINITIONS FOR THE AXES (INITIATING THE PLOT SIZES)
	left, width = 0.1, 0.65
